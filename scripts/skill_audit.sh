...
# Firmware/render/catalog are excluded from the antipattern scan (firmware MicroPython,
# linted at source in firmware/ — same exclusion as ruff/pyright).
set -uo pipefail

CC=custom_components/pimoroni_unicorn/
fail=0
FAIL() { echo "❌ FAIL: $*"; fail=1; }
WARN() { echo "⚠️  WARN: $*"; }

# --- Canonical workflows present ---
for w in create-dev-pr pr-labeler release_drafter release lint_pr \
         hacs_validate hassfest_validate python_validate check-manifest-version \
         firmware_test frontend_build; do
...
# --- Antipatterns in integration code (high-confidence; firmware copies excluded) ---
EX=(--include=*.py --exclude-dir=firmware --exclude-dir=render --exclude-dir=catalog)
ap() { grep -rnE "${EX[@]}" "$1" "$CC" 2>/dev/null && FAIL "$2"; }
...
ti=$(grep -rnE "${EX[@]}" '# type: ignore([^[]|$)' "$CC" 2>/dev/null)
[ -n "$ti" ] && { echo "$ti"; FAIL "bare # type: ignore without an error code (use # type: ignore[code])"; }
