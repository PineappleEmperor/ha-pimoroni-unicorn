...
if [ -n "$CC" ]; then
  ...
  ti=$(grep -rn '# type: ignore' "$CC" 2>/dev/null | grep -Ev 'import-untyped|/(render|catalog|firmware)/')
...
bad = []
GENERATED = {"render", "catalog", "firmware"}   # copies of firmware/, linted at source
for f in pathlib.Path(sys.argv[1]).rglob("*.py"):
    if GENERATED & set(f.parts):
        continue
    try:
