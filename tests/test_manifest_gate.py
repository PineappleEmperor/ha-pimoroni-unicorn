...
def test_chore_rides_in_cycle_minor() -> None:
    """A chore PR may ride an in-cycle minor already on main (the shipped regression)."""
    assert ok("1.1.0", "1.2.0", "1.2.0", ["chore"])

def test_chore_overbump_beyond_cycle_fails() -> None:
    """A chore PR bumping past the in-cycle version is rejected."""
...
def test_final_graduates_prerelease() -> None:
    """A final version graduates its own prerelease (2.0.0rc19 -> 2.0.0), even feature-labelled."""
...
def test_no_managed_label_passes_when_changed() -> None:
    """With no managed label the version only needs to differ from the last release."""
